...
class CrossEntropyCriterion(nn.Module):
    ...
    def forward(self, outputs, label, ignore_index):
        output = outputs["pred_masks"]
        output = F.interpolate(
            output, size=label.shape[-2:], mode="bilinear", align_corners=False
        )
        loss = F.cross_entropy(output, label, reduction="mean", ignore_index=ignore_index)
        losses = {'loss_ce': loss}  # loss must be prefix
